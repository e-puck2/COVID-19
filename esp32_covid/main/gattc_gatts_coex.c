...
#define INFECTED_PROBABILITY_STEP 100000000 // Increases the probability by step of 100000000/4294967295*100=2.3%


static char gatts_adv_name[16];
uint16_t robot_id;
uint8_t bt_change_name = BT_NAME_NO_CHANGE;
int64_t start_time = 0;
uint16_t current_id = 0;
uint32_t infected_probability = 0;
uint8_t infected_flag = 0;
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT:
		...
		// Restart advertising if it was stopped for changing the device name.
		if((bt_change_name != BT_NAME_NO_CHANGE) && (adv_config_done == 0)) {
    ...
    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
		...
		if(bt_change_name != BT_NAME_NO_CHANGE) {
			bt_change_name = BT_NAME_NO_CHANGE;
			
			ESP_LOGI(COEX_TAG, "Change BT name time = %lld us", esp_timer_get_time()-start_time);
		}
        break;
    case ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT:
		...
		// Change advertisement device name.
		if(bt_change_name == BT_NAME_CONTAGIOUS) {
			sprintf(gatts_adv_name, "e-puck2_%05d_C", robot_id);
		} else if(bt_change_name == BT_NAME_HEALTHY) {
			sprintf(gatts_adv_name, "e-puck2_%05d_H", robot_id);
		} else if(bt_change_name == BT_NAME_IMMUNE) {
			sprintf(gatts_adv_name, "e-puck2_%05d_M", robot_id);
		}
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
			...
			ESP_LOGI(COEX_TAG, "Detected id=%d", current_id);
            if (adv_name != NULL) {
				if ( strncmp((char *)adv_name, "e-puck2_", 8) == 0) { // An e-puck2 robot detected.
					if(scan_result->scan_rst.rssi > -83) { // The robot is near.
						bt_list_add(current_id); // Update list of robots heard.
						if((infected_flag == 0) && (actor_mode != MODE_PATIENT_ZERO)) { // If still not infected.
							if(adv_name[14]=='C') { // Current robot detected is contagious.
								if(infected_probability < (UINT32_MAX - INFECTED_PROBABILITY_STEP)) { // Increases probability each time a contagious robot is detected.
									infected_probability += INFECTED_PROBABILITY_STEP;
								}
								printf("infected prob = %d\r\n", infected_probability);
								if(esp_random() <= infected_probability) { // The probability of being infected increases with the number of times a contagious robot is detected in the near.
									infected_flag = 1;
									set_robot_infected();
									printf("infected!");
								}
...
void init_ble(void)
{
	...
	// Set the name of the device as seen by the computer; the zeros are replaced by the mac address.
	sprintf(gatts_adv_name, "e-puck2_%05d_H", robot_id);		
