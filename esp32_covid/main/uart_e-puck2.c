...
uint8_t get_selector(void) {
	return uart_rx_buff_last->data[89];
}

void set_speed(int16_t speed) {
	uart_tx_buff[3] = speed&0xFF; // Left speed / left steps LSB
	uart_tx_buff[4] = speed>>8; // Left speed / left steps MSB
	uart_tx_buff[5] = speed&0xFF; // Right speed / right steps LSB
	uart_tx_buff[6] = speed>>8; // Right speed / right steps MSB
}
...
void uart_init(void) {
	...
	xTaskCreate(&advsercom_task, "advsercom_task", 2048, NULL, 4, NULL);	
}
